...
from numpy import (array, ma, arctan2, sqrt, gradient, sign,
                   ...
                   zeros, zeros_like, pi, log, exp, deg2rad, warnings)
...
# Pour extrµaire le temps des photos
try:
...
class TigesManager():
    ...
    def add_pictures(self, image_names):
        """
        Function to add image to the tiges manager
    ...
    def select(self, idtiges):
        """
        Function to select some tige to process
        :param idtiges:
        :return:
        """
        pass
...
def plot_sequence(data, tige_color={1: "blue", 2: "green", 3: "red", 4: "cyan", 5: "orange", 6: "magenta", 7: "gray",
                                    8: "black", 9: "sandybrown"}, show_lims=True, ydata='angle', tige_alpha=0.5):
    """
    ...
    for i, tige in enumerate(Ntige):
        for sec in Nsec:
            ...
            try:
                data.query('sequence == %i and tige == %i' % (sec, tige)).plot('temps', ydata, ax=mpl.gca(),
                                                                               color=tige_color[i], alpha=tige_alpha,
                                                                               legend=False)
            except:
                pass
