...
def _export_to_csv():
    ...
    if len(outfileName) > 0:
        ...
        #print Ntige
        for tige in Ntige:
            datatige = dataout.query( 'tige == %i'%tige )
...
def _export_mean_to_csv():
    ...
    if len(outfileName) > 0:
        ...
        datamoy['tige'] = ['%i->%i'%(dataframe['tige'].min(), dataframe['tige'].max())]*len(datamoy['temps'])
...
def Traite_data():
    ...
    tiges_x, tiges_y, tiges_tailles, tiges_angles, tiges_lines, tiges_measure_zone = traite_tiges2( data_out['tiges_data'], return_estimation_zone=True )
...
def export_one_tige():
    ...
    if len(outfileName) > 0:
        ...
        tsmoothx, tsmoothy, tangle, ts, tN = traite_tige2( tx, ty, data_out['tiges_data'].diam[cur_tige,cur_image]/2.0, pas=0.3)
        tsmoothx = tsmoothx[:-1]
        tsmoothy = tsmoothy[:-1]
...
def show_one_tige(tige_id=None):
    ...
    if len(tps) > 1:
        ...
        tsmoothx, tsmoothy, tangle, ts, tN = traite_tige2( data_out['tiges_data'].xc[cur_tige,0], data_out['tiges_data'].yc[cur_tige,0], data_out['tiges_data'].diam[cur_tige,0]/2.0, pas=0.3)
...
def plot_moyenne():
    ...
    if len(files_to_process) > 1:
        ...
        if len(tige_id_mapper) == 0:
            ...
            legendtige=["tige %i"%tige_id_mapper[i] for i in tige_id_mapper]
            global_id=[tige_id_mapper]
...
def test_detection():
    ...
    if cur_image != None and len(base_tiges)>0 :
        ...
        Process_images( [files_to_process[cur_image]],'all', 1, base_points = [ base_tiges[good_tige] ], thread=False, show_tige=True)
...
def get_growth_length(tiges, cur_tige, thresold='auto', imgs = None, pas = 0.3):
    ...
    W = round( (tiges.diam[cur_tige].mean()/2.)/pas ) * 2.0
